...
class HOAUsersViewPermissions(BasePermission):
    """
    Comprehensive permissions class for HOAUser, based on which view is being requested
    ...
    # the various HOAUser views' permissionclasses will enforce all model permissions
    def has_permission(self, request, view):
        ...
        # will just check for 'Can view hoa user' rather than 'Can view ALL hoausers'
        if request.method == 'GET':
            return user_has_perm(request.user, 'Can view ALL hoausers', 'users.can_view_all_hoausers')

        # This override is necessary because the standard DjangoModelPermissions class
        # will require 'Can add hoa user'.  This is counterproductive because new users that
...
class HOAUserViewPermissions(BasePermission):
    # Model Permissions.  Use only as needed since DjangoModelPermissions in
    # the various HOAUser views' permissionclasses will enforce all model permissions.
    # Since a GET will only return a single user, that will be sufficient.  No need
    # to check for 'Can view ALL hoa users' here.
    def has_permission(self, request, view):
        # Deny actions on objects if the user is not authenticated
        if not request.user or not request.user.is_authenticated:
            return False
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method == 'GET':
            return obj == request.user or user_has_perm(request.user, 'Can view ALL hoausers', 'users.can_view_all_hoausers')
        elif request.method == 'PUT' or request.method == 'PATCH':
            return obj == request.user or user_has_perm(request.user, 'Can update ALL hoausers', 'users.can_update_all_hoausers')
        elif request.method == 'DELETE': # users with delete permission on the model can delete their own object
            return obj == request.user or user_has_perm(request.user, 'Can delete ALL hoausers', 'users.can_delete_all_hoausers')
        else:
            return False
...
#         if request.method == "GET":
#             return user_has_perm(request.user, 'Can view hoa user', 'users.view_hoauser')
#         return True
