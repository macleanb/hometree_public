  ...
  }) => 
{
  ...
  if (auth && auth.status && auth.status === constants.STATUS_AUTHENTICATED) {
    return (
      <section className="d-flex flex-column justify-content-start p-0 colorsettings_bodybackground heightsettings_residencepolicychoicescontainer">
            ...
            ?
              <Col>
                {residencePolicyChoicesDisplayArr.map((obj, index) => (
                  <Row key={ index } className="mb-5 justify-content-center w-100">
                    <PolicyChoiceCard
                      obj={ obj }
