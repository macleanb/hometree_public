...
export const getUserValidationErrors = (userData) => {
  let result = null;
...
/* Returns true if userData elements equal emptyUserData,
   otherwise false */
export const userDataIsEmpty = (userData) => {
  if (userData) {
    ...
    if (!('is_staff' in userData) || (userData.is_staff !== false)) {
      return false;
    }

    if (!('is_active' in userData) || (userData.is_active !== true)) {
      return false;
    }

    if (!('fk_mailing_address' in userData) || (userData.fk_mailing_address !== null)) {
      return false;
    }

    if (!('image' in userData) || (userData.image !== null)) {
      return false;
    }

    if (!('imageFileName' in userData) || (userData.imageFileName !== '')) {
      return false;
    }

    if (!('deleteExistingImage' in userData) || (userData.deleteExistingImage !== false)) {
      return false;
    }

    if (!('backendImageExists' in userData) || (userData.backendImageExists !== false)) {
      return false;
    }

    if (!('residences' in userData) || (userData.residences !== null)) {
      return false;
    }
